...
class TryOnPipeline:
    ...
    def _sample(
        ...
        *,
        ...
        images = torch.randn((1, c, h, w), dtype=dtype, device=device)
        ...
        for step_idx, (t_curr, t_prev) in enumerate(
            ...
            if step_idx >= num_timesteps - skip_cfg_last_n_steps:
                v_guided = v_c
            else:
        ...
        images = images.float().clamp_(-1.0, 1.0)

        return [tensor_to_pil(images[0], unnormalize=True)]
